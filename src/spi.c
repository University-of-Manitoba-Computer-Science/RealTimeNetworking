...
void init_spi()
{
    ...
    // PORT_REGS->GROUP[0].PORT_DIRSET |= WIFI0_CS_Msk;
    // PORT_REGS->GROUP[0].PORT_OUTSET |= WIFI0_CS_Msk;

    // configure SPI pins
    PORT_REGS->GROUP[0].PORT_PINCFG[WIFI0_MOSI] = PORT_PINCFG_PMUXEN_Msk;
    PORT_REGS->GROUP[0].PORT_PINCFG[WIFI0_MISO] = PORT_PINCFG_PMUXEN_Msk;
    PORT_REGS->GROUP[0].PORT_PINCFG[WIFI0_SCK] = PORT_PINCFG_PMUXEN_Msk;
    PORT_REGS->GROUP[0].PORT_PINCFG[WIFI0_CS] = PORT_PINCFG_PMUXEN_Msk;
    ...
    // PORT_REGS->GROUP[1].PORT_PMUX[2] = PORT_PMUX_PMUXO_C;

    // configure generic clock generator 3 to use the DFLL clock source
    GCLK_REGS->GCLK_GENCTRL[4] = GCLK_GENCTRL_SRC_DFLL | GCLK_GENCTRL_GENEN_Msk;
    while ((GCLK_REGS->GCLK_SYNCBUSY & GCLK_SYNCBUSY_GENCTRL_GCLK4) ==
           GCLK_SYNCBUSY_GENCTRL_GCLK4)
        ;

    GCLK_REGS->GCLK_PCHCTRL[SERCOM1_GCLK_ID_CORE] = GCLK_PCHCTRL_GEN_GCLK4 | GCLK_PCHCTRL_CHEN_Msk;
    ...
    SERCOM1_REGS->SPIM.SERCOM_BAUD = (uint8_t)SERCOM_SPIM_BAUD_BAUD(4);
